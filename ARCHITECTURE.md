...
| `WorkspaceLayoutManager.cs` | 版本化视觉快照、深复制、激活和只读聚合差异预览 |
...
- 工作区只保存视觉布局、显示器拓扑和 Portal 配置。差异预览直接比较当前根布局与目标快照的可见字段，不更新活动快照或时间戳；复制会深拷贝所选快照但不激活或应用；切换前先捕获当前快照，真实文件任务、拖动或规则执行期间禁止切换，恢复过程不调用文件系统写 API。
- 收件箱首次完整扫描只建立基线；后续新项目按稳定身份维护建议。显式批量接受只处理待处理、可靠且未受手工分组保护的项目，并只提交一次布局保存；低可靠度和稍后处理结果保持原位。收件箱标签编辑只替换 `ItemTags` 中该项目的本地集合，不消费队列项；加载、导入、规则追加和人工编辑共用同一规范化策略。搜索只投影 `_desktopItems` 及本地布局元数据，不做全盘/正文索引。
