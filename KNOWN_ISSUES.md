...
## 分类器测试缺少 System.IO 导入，完整构建和 CI 门禁失败

- 现象：规定的 Release 严格构建中，应用项目生成成功，但测试项目出现 9 个 `CS0103`/`CS0246` 编译错误，无法解析 `Path`、`File`、`Directory` 和 `IOException`。
- 影响：解决方案构建失败，自动化测试、覆盖率采集和后续 CI 发布步骤均无法执行。
- 证据或复现方式：在干净 `main` 运行 `dotnet build DesktopOrganizer.slnx -c Release -warnaserror`，当前结果为 0 个警告、9 个错误。`tests/DesktopOrganizer.Tests/DesktopCategoryClassifierTests.cs:1` 只显式导入 MSTest，却在 `:36-94` 使用 `System.IO` 类型；测试项目的隐式全局导入不包含 `System.IO`，主项目 `GlobalUsings.cs:4` 的导入不会跨项目引用传递。
- 涉及区域：测试项目编译、解决方案构建、`scripts/ci.ps1`、Windows CI。
